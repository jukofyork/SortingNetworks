...
int CompSucc(const void* E1,const void* E2)
{ // For qsort.
...
int BeamSearch(State &Result)
{ // Finds a sorting network by slowly reducing the depth of the tree
  ...
  int SuccOps[NET_SIZE][NET_SIZE];       // List of opertion allowed.
  int    NumSuccs;                         // Number of successors.

  // Do until one is found.
  for (int Level=0;;Level++) {

    cout << Level << ' '; cout.flush();
    ...
    for (I=0;I<BEAM_SIZE;I++) {
	  ...
	  if (USE_ASYMITRY_HEURISTIC==true && Level>=1
		   ...
		   *TempStatePtr=BeamState;
		   TempStatePtr->UpdateState((NET_SIZE-1)-N2,(NET_SIZE-1)-N1);  // Update it.
